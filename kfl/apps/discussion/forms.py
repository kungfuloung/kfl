...
class ForumSearchForm_en(forms.Form):
    category = forms.ChoiceField(choices = ((0, 'All Forums'), (1,'Master Asks'), (11,'Martial Arts Forum'), (12,'Qi Forum'), (13,'Weaponry Forum')))
...
class ForumSearchForm_cn(forms.Form):   
    category = forms.ChoiceField(choices = ((0, '所有論壇'), (1,'師父問'), (11,'武術答疑區'), (12,'氣功答疑區'), (13,'兵器答疑區')))
    language = forms.ChoiceField(choices = ((0, '所有語言'), (1,'英文'), (2,'中文')))
    name = forms.CharField(required=False, widget=forms.TextInput(attrs={'size':14}))


class ThreadPostForm(forms.Form):
    title = forms.CharField(required=True, widget=forms.Textarea(attrs={'rows':2, 'cols':100})) 
    language = forms.ChoiceField(choices = ((1,'English'), (2,'中文')))   
    content = forms.CharField(required=True, widget=forms.Textarea(attrs={'rows':20, 'cols':100}))  
